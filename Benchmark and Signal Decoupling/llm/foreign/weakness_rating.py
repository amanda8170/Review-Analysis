...
OUTPUT_FILE = '../../../../Datasets/predict_result/llm/test/gemini/weakness_rating.jsonl'
...
def get_prediction_from_llm(content: str, scores: List[int], max_retries: int = 3) -> str:
    ...
    prompt_template = """Drawing upon your extensive knowledge of ICLR conference standards and historical acceptance trends, synthase the quantitative Ratings with the qualitative Weaknesses provided to predict paper's final decision.
1.  Data Constraint: The provided text consists solely of the "Weakness" sections from reviews.
2.	Rating Scale: 10 (Strong Accept), 8 (Accept), 6 (Marginally Accept), 5 (Marginally Reject), 3 (Reject), 1 (Strong Reject).
Reviewer Ratings: {scores_list}
Reviewer Weaknesses: {content}
...
def format_output_data(original_paper: Dict, scores: List[int], extracted_weaknesses: str, model_raw_output: str,
    ...
    return {
        ...
        "input_extracted_weaknesses": extracted_weaknesses[:200] + "...",  # 预览
...
def predict_and_save(papers_to_process: List[Dict], output_file: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for paper in tqdm(papers_to_process, desc="Processing"):
            ...
            # 1. 提取 Weakness Only
            combined_weaknesses = extract_all_reviews_weaknesses(paper.get('reviews', []))
            ...
            # 如果没有分数或文本，跳过
            if not scores or not combined_weaknesses:
                continue

            # 3. 预测 (传入 Weakness + Rating)
            model_raw_output = get_prediction_from_llm(combined_weaknesses, scores)

            # 4. 解析
            predict_decision = parse_prediction(model_raw_output)

            # 5. 保存
            output_data = format_output_data(paper, scores, combined_weaknesses, model_raw_output, predict_decision)
