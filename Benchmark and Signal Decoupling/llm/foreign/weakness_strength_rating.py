...
OUTPUT_FILE = '../../../../Datasets/predict_result/llm/test/gemini/weakness_strength_rating.jsonl'
...
Do not provide any reasons or explanations."""
    ...
    prompt = prompt_template.format(
        review_strengths_and_weaknesses=content,
        scores_list=str(scores)
    )
...
def predict_and_save(papers_to_process: List[Dict], output_file: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for paper in tqdm(papers_to_process, desc="Processing"):
            ...
            # 如果缺少分数或文本，跳过
            if not scores or not combined_sw:
                continue

            # 3. 预测 (传入文本 + 分数)
            model_raw_output = get_prediction_from_llm(combined_sw, scores)
