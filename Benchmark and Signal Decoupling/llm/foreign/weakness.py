...
OUTPUT_FILE = '../../../../Datasets/predict_result/llm/test/gemini/weakness.jsonl'
...
def get_prediction_from_llm(content: str, max_retries: int = 3) -> str:
    ...
    prompt_template = """Drawing upon your extensive knowledge of ICLR conference standards and historical acceptance trends, analyze the provided Weaknesses from the initial reviews to predict the final decision for this paper.
Provided Weaknesses: {review_weaknesses} 
You must output the result strictly in the following format:
Decision: [Accept or Reject]
Do not provide any reasons or explanations."""

    # 只传入 weakness 文本
    prompt = prompt_template.format(review_weaknesses=content)
...
def format_output_data(original_paper: Dict, scores: List[int], extracted_content: str, model_raw_output: str,
    ...
    return {
        ...
        "rating": scores,  # 记录分数作为参考
        "real_decision": original_paper.get("paper_decision", ""),
        "input_extracted_weaknesses": extracted_content[:200] + "...",  # 预览
...
def predict_and_save(papers_to_process: List[Dict], output_file: str):
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for paper in tqdm(papers_to_process, desc="Processing"):
            ...
            # 如果没有提取到任何 Weakness，跳过
            if not combined_weaknesses:
                continue

            # 3. 预测 (仅传入 Weakness 文本)
            model_raw_output = get_prediction_from_llm(combined_weaknesses)
