...
OUTPUT_FILE = '../../../../Datasets/predict_result/llm/test/Qwen/rating.jsonl'
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                print(f"API返回错误 (尝试 {attempt + 1}/{max_retries}): {response.code} - {response.message}")
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
        except Exception as e:
            print(f"发生网络或SDK异常 (尝试 {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)
