...
# --- NEW FUNCTION 1: 提取文本中的 Strengths 和 Weaknesses ---
...
# --- NEW FUNCTION 2: 遍历所有 Reviewer 并提取 S&W ---
...
def prepare_dataset(data: List[Dict]) -> Tuple[List[str], List[str], List[Dict]]:
    ...
    for paper in data:
        # --- MODIFIED: 调用新的提取函数，传入 reviews 列表 ---
        reviews_list = paper.get('reviews', [])
        earliest_review_content = extract_all_reviews_sw(reviews_list)
...
def create_detailed_predictions_jsonl(
    ...
    for i, paper in enumerate(valid_papers):
        paper_data = {
            ...
            'reviews': [],
            'method': 'TFIDF_strength_weakness'
        }
...
def main():
    ...
    print("Preparing datasets (using S&W extraction)...")
    ...
    if test_data_valid:
        ...
        if detailed_jsonl:
            ...
            output_filename = '../../../Datasets/predict_result_new/strength_weakness/TFIDF.jsonl'
