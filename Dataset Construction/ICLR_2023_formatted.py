...
class OpenReviewProcessor:
    ...
    # --- 2. 修改: extract_paper_info 使用新函数和正确的数据格式 ---
    ...
    def extract_decision(self, data: Dict) -> str:
        ...
        for review in reviews:
            ...
            # 检查 review 内部的 invitation 是否包含 desk reject
    ...
    def extract_review_content(self, review: Dict) -> str:
        ...
        if 'Reviewer' in signature:
            ...
            if content.get('strength_and_weaknesses'):
                parts.append(f"Strengths and Weaknesses: {content['strength_and_weaknesses']}")
            if content.get('clarity,_quality,_novelty_and_reproducibility'):
                parts.append(
                    f"Clarity,quality,novelty and reproducibility: {content['clarity,_quality,_novelty_and_reproducibility']}")
            ...
            if 'metareview:_summary,_strengths_and_weaknesses' in content:
                parts = []
                summary_text = content.get('metareview:_summary,_strengths_and_weaknesses')
                if summary_text:
                    parts.append(f"Metareview Summary:\n{summary_text}")
                higher_score_justification = content.get('justification_for_why_not_higher_score')
                if higher_score_justification:
                    parts.append(f"Justification for Why Not Higher Score:\n{higher_score_justification}")
                lower_score_justification = content.get('justification_for_why_not_lower_score')
                if lower_score_justification:
                    parts.append(f"Justification for Why Not Lower Score:\n{lower_score_justification}")
                summary_of_reviewer_meeting = content.get('summary_of_AC-reviewer_meeting')
                if summary_of_reviewer_meeting:
                    parts.append(f"Summary of AC-reviewer meeting: {summary_of_reviewer_meeting}")
                return "\n\n".join(parts)
    ...
    def process_dataset(self, input_file: str, output_file: str):
        ...
        with jsonlines.open(output_file, 'w') as writer:
            for paper_data in all_papers:
                ...
                # --- c. 写入数据并进行处理后统计 ---
                # 定义此数据集可接受的最终决定
                decisions_to_keep = ['Accept: notable-top-5%', 'Accept: poster', 'Accept: notable-top-25%', 'Reject']
...
def main():
    ...
    input_filename = '../../Datasets/raw_dataset/ICLR.cc_2023.jsonl'
    # 只需要一个输出文件名
    output_filename = '../../Datasets/formatted_dataset/ICLR.cc_2023_formatted.jsonl'
