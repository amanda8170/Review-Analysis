...
def extract_all_reviews_weaknesses(reviews: List[Dict]) -> str:
    ...
    for review in reviews:
        ...
        if valid_items:
            ...
            if weakness_text:
                # 注释掉了添加 reviewer_id 的部分，以确保输入模型的纯粹是文本内容
...
def prepare_data(data: List[Dict]) -> Tuple[List[str], List[int], List[Dict]]:
    ...
    for item in data:
        # --- 【核心修改点】使用新的函数只提取 Weaknesses 作为输入 ---
...
def main():
    ...
    output_filename = '../../../Datasets/predict_result_new/weakness/CNN.jsonl'
    model_path = 'TextCNN_weakness_best_model.pth'
    ...
    # 3. 提取特征
    print("Preparing data (Extracting Weaknesses only)...") # 更新了打印信息
