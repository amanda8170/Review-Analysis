...
#
        ...
        for review in paper.get('reviews', []):
            # To be consistent with previous logic, we only use reviewer comments
            reviewer_id = review.get('reviewer', '')
            if any(role in reviewer_id for role in ['Program_Chair', 'Area_Chair', 'Author']):
                continue

            for dialogue in review.get('dialogue', []):
...
def main():
    ...
    ### MODIFICATION: 更改为本地文件路径 ###
    # 请确保你的数据文件放在这个'./data/'目录下
    DATA_DIR = '../../../Datasets/formatted_dataset/'
    OUTPUT_DIR = '../../../Datasets/predict_result_new/rating_initial review_rebuttal/'
    # 如果输出目录不存在，则创建它
